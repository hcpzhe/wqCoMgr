...
class OrderModel extends Model{
	...
	Public function orderlist($where){
		...
		$data['order_list']=$this->table('erp_order as oe,erp_user as ur,erp_customer as cr,erp_product as pt')
					->where($where)
					->field("oe.id as id,ur.realname as uname,cr.id as cust_id,cr.`name` as cname,pt.`name` as pname,pt.`id` as pid,oe.total_fees,oe.signed_time as signed_time,oe.`check`")
	...
	public function orderinfo($id){
		...
		->field("oe.id as id,cr.phone as phone,cr.fax as fx,cr.contacts as contacts,ur.realname as uname,cr.`name` as cname,pt.`name` as pname,oe.total_fees as money,oe.status as status,oe.remark,oe.expired_time extime,signed_time as signed_time,oe.`check`,oe.time_limit as limitime")->find();
